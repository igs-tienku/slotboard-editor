...
test("build emits the GitHub Pages-compatible application", async () => {
  ...
  assert.match(html, /app\.js\?v=0\.20\.1/);
  assert.match(html, /app\.css\?v=0\.20\.1/);
...
test("M6.1 editor keeps safe transforms, context actions and production exports available", async () => {
  ...
  assert.match(editor, /sceneZoomRef\.current = next/);
  assert.match(editor, /saved && !userInteractedRef\.current/);
  assert.match(editor, /onPointerDownCapture=\{\(\) => \{ userInteractedRef\.current = true; \}\}/);
  assert.match(editor, /window\.addEventListener\("pointerup", finishDrag, true\)/);
  assert.match(editor, /window\.addEventListener\("pointercancel", cancelDrag, true\)/);
  assert.match(editor, /document\.addEventListener\("visibilitychange", cancelWhenHidden\)/);
  assert.match(editor, /onWheelCapture=\{releaseNumberInputWheel\}/);
  assert.match(editor, /shouldReleaseNumberInputForWheel/);
  assert.match(editor, /m2-align-actions/);
  ...
  assert.match(editor, /strokeWidth=\{24\}/);
  ...
  assert.match(editor, /interactionId=\{effectiveInteractionId\}/);
  assert.match(editor, /className="m20-group-hit-target"/);
  assert.match(editor, /pointerEvents="all"/);
  assert.match(editor, /onPointerDown=\{\(event\) => onSelect\(event, effectiveInteractionId\)\}/);
  assert.match(editor, /baseChildren: layer\.type === "group"/);
  assert.match(editor, /scaleGroupChildren\(drag\.baseChildren/);
