...
test("build emits the GitHub Pages-compatible application", async () => {
  ...
  assert.match(html, /app\.css\?v=0\.22\.0/);
...
test("M6.1 editor keeps safe transforms, context actions and production exports available", async () => {
  ...
  assert.match(editor, /defaultFlowCardSize\(scene\)/);
  assert.match(editor, /from\.x \+ fromSize\.width/);
  assert.match(editor, /from\.y \+ fromSize\.height \/ 2/);
  assert.match(editor, /m22-flow-card-resize/);
  assert.match(editor, /nextFlowCardSize/);
  assert.match(editor, /event\.shiftKey/);
  assert.match(editor, /完整畫布預覽/);
  assert.match(css, /\.flow-card-preview \.m3-scene-thumbnail\s*\{[^}]*object-fit:\s*contain/s);
  assert.match(css, /\.m3-flow-card\s*\{[^}]*grid-template-rows:\s*minmax\(0, 1fr\)/s);
