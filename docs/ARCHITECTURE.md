...
完成紀錄（2026-08-13）：已實作畫布／多物件對齊、水平／垂直等距、8px 智慧吸附、動態參考線、像素格線、數值面板、20 MB／8192px／200 MB 容量規則、超標縮小副本、原地圖片置換、contain／cover、焦點、縮放與 placeholder 重設。恢復草稿升級至 IndexedDB，可保存圖片資料與裁切設定；文字工具涵蓋三類隨產品提供的 Noto OFL-1.1 WOFF2 子集、內容、字級、字重、斜體、行距、字距、水平／垂直對齊、外框與底色。圖片裁切序列化／重開測試通過。
...
閉環補強紀錄（0.18.0）：流程卡片拖曳改為單一 undo 交易，Scene／流程雙軸拖曳數學有固定回歸；Scene 複製與模板匯入會重映射標註、Symbol 及素材 ID；ZIP 於解壓前依中央目錄資訊執行容量預檢；主畫面與 Worker 共用內建字型載入器。這些自動證據不替代 `docs/M6-QA.md` 所列瀏覽器、Photoshop 與效率人工驗收。
