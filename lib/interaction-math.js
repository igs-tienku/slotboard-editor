...
export function nextFlowCardSize(origin, startX, startY, clientX, clientY, zoom, lockAspect = false, limits = {}) {
  const delta = clientDragDelta(startX, startY, clientX, clientY, zoom);
  let width = origin.width + delta.x, height = origin.height + delta.y;
  if (lockAspect) {
    const ratio = origin.width / origin.height;
    if (Math.abs(delta.x) >= Math.abs(delta.y)) height = width / ratio;
    else width = height * ratio;
  }
  const minWidth = limits.minWidth ?? 200, maxWidth = limits.maxWidth ?? 640;
  const minHeight = limits.minHeight ?? 180, maxHeight = limits.maxHeight ?? 600;
  return {
...
export function shouldReleaseNumberInputForWheel(inputType, isFocused) {
  return inputType === "number" && isFocused;
}

export function clampLayerTransformValue(property, value, transform, scene) {
  const numeric = Number(value);
  if (!Number.isFinite(numeric)) return transform[property];
  if (property === "x") return Math.round(Math.max(0, Math.min(scene.width - transform.width, numeric)));
  if (property === "y") return Math.round(Math.max(0, Math.min(scene.height - transform.height, numeric)));
  if (property === "width") return Math.round(Math.max(1, Math.min(scene.width - transform.x, numeric)));
  if (property === "height") return Math.round(Math.max(1, Math.min(scene.height - transform.y, numeric)));
  return numeric;
}
