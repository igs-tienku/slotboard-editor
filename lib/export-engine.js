...
export function psdLayerSequence(layers, scene) {
  // ag-psd's written record order is displayed in reverse by Krita's PSD
  // importer. Supply bottom-to-top records so the opened PSD matches the
  // editor's top-to-bottom layer panel at every nesting level.
  if (!scene) return [...layers].reverse();
  ...
  return [...content, ...backgrounds].reverse();
}

function makeCanvas(width, height) {
  const canvas = typeof document !== "undefined" ? document.createElement("canvas") : new OffscreenCanvas(Math.max(1, Math.ceil(width)), Math.max(1, Math.ceil(height)));
...
function loadImage(dataUrl) {
  if (typeof Image !== "undefined") return new Promise((resolve, reject) => { const image = new Image(); image.onload = () => resolve(image); image.onerror = reject; image.src = dataUrl; });
  return fetch(dataUrl).then((response) => response.blob()).then((blob) => createImageBitmap(blob));
}

async function canvasPngBytes(canvas) {
  if (typeof canvas.convertToBlob === "function") return new Uint8Array(await (await canvas.convertToBlob({ type: "image/png" })).arrayBuffer());
...
async function buildPsdLayer(layer, scene, project, ancestorTransforms = []) {
  if (layer.type === "group") {
    const children = [];
    for (const child of psdLayerSequence(layer.children)) children.push(await buildPsdLayer(child, scene, project, [...ancestorTransforms, layer.transform]));
    return { name: layer.name, hidden: !layer.visible, opacity: layer.opacity, opened: layer.opened !== false, children };
  }
  ...
  context.restore();
  const full = context.getImageData(0, 0, scene.width, scene.height);
  let left = scene.width, top = scene.height, right = -1, bottom = -1;
  for (let y = 0; y < scene.height; y += 1) for (let x = 0; x < scene.width; x += 1) {
    if (full.data[(y * scene.width + x) * 4 + 3] === 0) continue;
    left = Math.min(left, x); top = Math.min(top, y); right = Math.max(right, x); bottom = Math.max(bottom, y);
  }
  if (right < left || bottom < top) return { name: layer.name, hidden: !layer.visible, opacity: layer.opacity, left: 0, top: 0, right: 1, bottom: 1, imageData: context.getImageData(0, 0, 1, 1) };
  const width = right - left + 1, height = bottom - top + 1;
  return { name: layer.name, hidden: !layer.visible, opacity: layer.opacity, left, top, right: right + 1, bottom: bottom + 1, imageData: context.getImageData(left, top, width, height) };
}
...
export async function createScenePsd(project, sceneId) {
  ...
  const children = [];
  for (const layer of psdLayerSequence(scene.layers, scene)) children.push(await buildPsdLayer(layer, scene, project));
  const documentData = { width: scene.width, height: scene.height, imageData: composite.getContext("2d").getImageData(0, 0, scene.width, scene.height), children };
  return globalThis.agPsd.writePsdUint8Array(documentData, { generateThumbnail: false });
}

function countLeafLayers(layers) {
  return layers.reduce((sum, layer) => sum + (layer.type === "group" ? countLeafLayers(layer.children) : 1), 0);
}

export function estimateExportWorkingSet(project) {
  let psdPeakBytes = 0, pdfPeakBytes = 0, totalLayers = 0;
  project.sceneOrder.forEach((sceneId) => {
    const scene = project.scenes[sceneId], pixels = scene.width * scene.height * 4, leaves = countLeafLayers(scene.layers);
    totalLayers += leaves;
    psdPeakBytes = Math.max(psdPeakBytes, pixels * 3);
    pdfPeakBytes = Math.max(pdfPeakBytes, pixels + (scene.width + 420) * Math.max(scene.height, 600) * 4);
  });
  return { psdPeakBytes, pdfPeakBytes, assetBytes: Object.values(project.assets ?? {}).reduce((sum, asset) => sum + (asset.byteLength ?? 0), 0), totalLayers };
}
...
export async function createProjectPdf(project) {
  ...
  for (const canvas of canvases) { const png = await pdf.embedPng(await canvasPngBytes(canvas)); const page = pdf.addPage([canvas.width, canvas.height]); page.drawImage(png, { x: 0, y: 0, width: canvas.width, height: canvas.height }); }
  return pdf.save();
}
