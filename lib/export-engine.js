...
async function buildPsdLayer(layer, scene, project) {
  if (layer.type === "group") return { name: layer.name, hidden: !layer.visible, opacity: layer.opacity, opened: layer.opened !== false, children: await Promise.all(psdLayerSequence(layer.children).map((child) => buildPsdLayer(child, scene, project))) };
