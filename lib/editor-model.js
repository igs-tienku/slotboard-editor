...
function initializeDiagramGeometry(project) {
  Object.values(project.scenes ?? {}).forEach((scene) => {
    ...
    (scene.annotations ??= []).forEach((annotation) => {
      ...
      if (legacyGeometry && placed.some((item) => annotation.x < item.x + item.width + 18 && annotation.x + annotation.width + 18 > item.x && annotation.y < item.y + item.height + 18 && annotation.y + annotation.height + 18 > item.y)) {
        const rows = Math.max(1, Math.floor((scene.height - 60) / (annotation.height + 18)));
        for (let slot = 0; slot < 200; slot += 1) {
          const x = scene.width + 50 + Math.floor(slot / rows) * (annotation.width + 18);
          const y = 50 + (slot % rows) * (annotation.height + 18);
          if (!placed.some((item) => x < item.x + item.width + 18 && x + annotation.width + 18 > item.x && y < item.y + item.height + 18 && y + annotation.height + 18 > item.y)) {
            annotation.x = x; annotation.y = y; break;
          }
        }
      }
...
export function updateSceneOverview(project, sceneId, position) {
  ...
  next.scenes[sceneId].overview = {
    ...next.scenes[sceneId].overview,
    x: Math.max(-FLOW_WORKSPACE_LIMIT, Math.min(FLOW_WORKSPACE_LIMIT, Math.round(position.x))),
    y: Math.max(-FLOW_WORKSPACE_LIMIT, Math.min(FLOW_WORKSPACE_LIMIT, Math.round(position.y))),
  };
  return touch(next);
}
...
export function autoArrangeScenes(project) {
  ...
  next.sceneOrder.forEach((id) => {
    const column = level.get(id), row = rows.get(column) ?? 0;
    next.scenes[id].overview = { x: 80 + column * 280, y: 80 + row * 180 };
...
export function addAnnotation(project, sceneId, text = "新增標註", targetLayerId = null) {
  return updateScene(project, sceneId, (scene) => {
    const width = DEFAULT_ANNOTATION_SIZE.width, height = DEFAULT_ANNOTATION_SIZE.height, gap = 18;
    const rows = Math.max(1, Math.floor((scene.height - 60) / (height + gap)));
    let slot = 0, x = scene.width + 50, y = 50;
    while (slot < 200) {
      const row = slot % rows, column = Math.floor(slot / rows);
      x = scene.width + 50 + column * (width + gap);
      y = 50 + row * (height + gap);
      const occupied = scene.annotations.some((item) => {
...
export function moveAnnotation(project, sceneId, annotationId, position) {
  ...
  return updateAnnotation(project, sceneId, annotationId, {
    x: Math.round(Math.max(0, Math.min(scene.width + 6000, position.x))),
