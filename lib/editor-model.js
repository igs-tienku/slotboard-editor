...
export function applyBuiltInSceneTemplate(scene, template = "basic") {
  const background = createShapeLayer("rectangle", 1, {
    name: "背景", fill: "#4c4d49", stroke: "#4c4d49",
...
export function createProject(name = "SlotBoard 專案", size = DEFAULT_SCENE_SIZE, template = "basic") {
  ...
  return {
    ...
    editorSettings: { snap: true, guides: true, pixelGrid: false, snapDistance: 8, flowZoom: 1 },
  };
}
...
export function migrateProject(input) {
  ...
  if (Number(input.schemaVersion) > SCHEMA_VERSION) {
    ...
    readonly.symbols ??= {};
    readonly.assets ??= {};
    readonly.editorSettings ??= { snap: true, guides: true, pixelGrid: false, snapDistance: 8, flowZoom: 1 };
  ...
  if (input.schemaVersion === 1) {
    ...
    migrated.editorSettings = { snap: true, guides: true, pixelGrid: false, snapDistance: 8, flowZoom: 1 };
    return normalizeBackgroundOrder(migrated);
  }
...
export function updateSceneOverview(project, sceneId, position) {
  const next = cloneProject(project);
  if (!next.scenes[sceneId]) return project;
  next.scenes[sceneId].overview = { ...next.scenes[sceneId].overview, x: Math.max(20, Math.round(position.x)), y: Math.max(20, Math.round(position.y)) };
  return touch(next);
}
...
export function moveAnnotation(project, sceneId, annotationId, position) {
  ...
  return updateAnnotation(project, sceneId, annotationId, {
    x: Math.round(Math.max(0, Math.min(scene.width + 170, position.x))),
    y: Math.round(Math.max(0, Math.min(scene.height - 120, position.y))),
  });
}
