...
function makeManifest(kind, source, assetIndex) {
  return {
...
function extractAssets(project) {
  ...
  Object.values(copy.assets ?? {}).forEach((asset) => {
    ...
    files[path] = bytes;
    index.push({ id: asset.id, path, mimeType, byteLength: bytes.length });
...
export function createProjectPackage(project) {
  ...
  files["manifest.json"] = strToU8(JSON.stringify(makeManifest("project", project, index), null, 2));
...
export function createTemplatePackage(project, sceneId) {
  ...
  const source = { ...project, name: scene.name, scenes: { [scene.id]: scene }, sceneOrder: [scene.id], connections: [] };
  const { copy, files, index } = extractAssets(source);
  files["manifest.json"] = strToU8(JSON.stringify(makeManifest("template", source, index), null, 2));
...
function readPackage(bytes, expectedKind) {
  const files = unzipSync(bytes, { filter: (file) => !file.name.endsWith("/") });
...
function restoreAssets(container, files, manifest) {
  for (const entry of manifest.assets ?? []) {
    ...
    if (!bytes || bytes.length !== entry.byteLength) throw new Error(`Missing or damaged asset: ${entry.id}`);
