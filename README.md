...
此工作區包含 SlotBoard MVP 的產品需求、技術架構、PSD 輸出技術原型，以及已完成並部署的 M1–M22 編輯器。
...
- v0.21.1 修正 Scene 標註從 HTML 控制列起拖時混用瀏覽器座標與 SVG 座標，導致按下瞬間跳位的問題。
- v0.21.2 固定 Scene 備註欄寬度並凍結單次拖曳使用的 SVG 座標矩陣，消除拖曳造成欄位擴張、比例回授與異常靈敏；超出舊範圍的標註會安全移回固定畫布。

## M22 流程 Scene 完整預覽與卡片縮放

- 流程卡片預設依 Scene 原始長寬比決定高度；橫版、直版、方形與超寬畫布皆以 `contain` 顯示完整畫布範圍，不裁切或拉伸。
- 預覽區成為卡片主要區域，並顯示 Scene 名稱、原始尺寸、圖層數與標註數，便於在大型流程中辨識畫面。
- 每張 Scene 卡片可由右下角獨立縮放，Shift 可維持卡片比例；尺寸限制為 200×180 至 640×600，並隨 `.slotboard` 儲存。
- 卡片拖曳與縮放都會依流程縮放倍率修正手感，單次操作只建立一筆 Undo；連線端點會即時跟隨卡片實際尺寸。
- 自動整理改用各欄最大卡片寬度與每張卡片高度計算間距，避免不同尺寸的 Scene 卡互相覆蓋。
